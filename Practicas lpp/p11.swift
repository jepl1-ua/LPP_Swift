...
struct Cuadrado {
 ...
 let cuadrado = Cuadrado(esquina: self.esquina.movida(incX: incX, 
incY: incY),
 lado: self.lado)
 return cuadrado
 }
...
print("""
La versión 1 crea un nuevo cuadrado en el que usa como esquina una esquina 
nueva 
que devuelve el método "movida" aplicado sobre la esquina actual.
La versión 2 crea también un nuevo cuadrado (al hacer la asignación se hace 
una
...
// 
print("""
c) La función `foo` es una función que devuelve `true` si
longitud de la cadena es mayor o igual que el parámetro que
se le pasa. Se hace un map de una clausura en la que se instancia
ese parámetro a 4, por lo que devuelve un array de booleanos
indicando si la palabra original tiene una longitud mayor o igual
que 4.
""")
/*
===========
...
struct Valor {
 ...
 // actualiza el valor de la propiedad z de Valor (propiedad del 
 //tipo)
 Valor.z += newValue
 } 
 didSet {
...
var partidos = [Partido(local:"Real Madrid", golesLocal:0,
 ...
 Partido(local:"Liverpool", golesLocal:4, 
visitante:"Barcelona", golesVisitante:0)]
...
- ¿Se puede sobreescribir el valor de una propiedad almacenada? ¿Y calculada?
 Se pueden sobreescribir los getters y setters. Si la propiedad padre es 
de solo
 lectura, la propiedad sobreescrita puede ser de lectura y escritura. Una 
propiedad
 padre de lectura-escritura no se puede sobreescribir con una de sólo 
lectura.
 Se puede comprobar en las variables 'a' y 'x' de la clase derivada.
- ¿Se puede añadir un observador a una propiedad de la clase base en una 
clase derivada?
...
- ¿Hereda la clase derivada propiedades y métodos estáticos de la clase base?
 Sí. Lo hemos comprobado accediendo a valor e incrementa() en la clase 
derivada
- ¿Cómo se puede llamar a la implementación de un método de la clase base en 
una sobreescritura 
 de ese mismo método en la clase derivada?
 Usando 'super'. Lo hemos hecho en el método 'suma()' de la clase 
derivada.
*/
/*
================================
...
func areaTriangulo(_ p1: Punto, _ p2: Punto, _ p3: Punto) -> Double {
 let det = p1.x * (p2.y - p3.y) + p2.x * (p3.y - p1.y) + p3.x * (p1.y -
p2.y)
 return abs(det)/2
}
struct Punto {
...
class Cuadrilatero: Figura {
 ...
 let alto = maxY - minY
 let ancho = maxX - minX
 super.init(origen: Punto(x: minX, y: minY), tamaño: Tamaño(ancho: 
ancho, alto: alto))
 }
}
